...
def get_all_heroku_teams():
    headers = {
        "Authorization": f"Bearer {HEROKU_API}",
        "Accept": "application/vnd.heroku+json; version=3",
    }

    # Fetch teams using Heroku API
...
@app.on_message(filters.command("teams") & filters.user(OWNER_ID))
async def heroku_teams_command(client, message):
    try:
        team_names = get_all_heroku_teams()
        if team_names:
            response = "\n".join(team_names)
        else:
            response = "No Heroku Teams found for the account."
        await message.reply_text(response)
    except Exception as e:
        await message.reply_text(f"Error fetching Heroku Teams: {str(e)}")
